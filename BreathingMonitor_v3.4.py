    ...
    def run(self):
        ...
        while not self.stopped:
            ...
            if current_time - last_capture_time >= self.capture_interval:
                ret, frame = cap.read()
                if ret:
                    self.shared_data.update_frame(current_time, frame)
...
class BreathingMonitorApp:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Breathing Monitor v3.4")
    ...
    def update_graph(self):
        ...
        if self.x_vals and self.y_vals:
            # Apply calibration and shifting
            y_vals_calibrated = [
            ...
            if self.x_vals[-1] > self.graph_window_sec:
                start_time = self.x_vals[-1] - self.graph_window_sec
                self.ax.set_xlim(start_time, self.x_vals[-1])
            else:
                self.ax.set_xlim(0, self.graph_window_sec)

            # Update y-axis limits
            visible_y_vals = y_vals_calibrated[-int(self.graph_window_sec * self.sample_rate):]
            if visible_y_vals:
            ...
            # Update background color based on current amplitude
